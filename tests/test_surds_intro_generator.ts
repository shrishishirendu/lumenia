import { generateQuestion, generatePool, generateMixedPool } from "../server/services/questionEngine/surdsIntro";
...
console.log("=== Surds Intro Generator Tests ===\n");
...
console.log("\n5. Mixed pool works");
{
  ...
  assert(mixed.length === 6, `Mixed pool should have 6 questions, got ${mixed.length}`);
  const ids = new Set(mixed.map(q => q.id));
  assert(ids.size === mixed.length, `Mixed pool should have unique IDs`);
}

console.log("\n6. Rationalisation questions exist in hard/challenge");
{
  ...
  const hasRationalise = allPool.some(q =>
    q.metadata.skills.includes("rationalise_denominator") ||
    q.metadata.skills.includes("conjugate_rationalisation")
  );
  assert(hasRationalise, "At least one rationalisation question should exist in hard/challenge pools");
}
...
console.log("\n9. Skill tags validation");
{
  const validSkills = new Set([
    "identify_surd", "simplify_surd", "combine_like_surds", "multiply_surds",
    "expand_brackets", "difference_of_squares", "rationalise_denominator",
    "conjugate_rationalisation",
  ]);
  for (const diff of DIFFICULTIES) {
    const pool = generatePool(diff, 10, 42);
    for (const q of pool) {
