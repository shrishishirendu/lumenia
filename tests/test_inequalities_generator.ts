import { generateQuestion, generatePool, generateMixedPool, type GeneratedQuestion } from "../server/services/questionEngine/inequalities";
...
function assertKeys(q: GeneratedQuestion, diff: string) {
  ...
  assert(q.topic === "inequalities", `${diff}: topic is 'inequalities'`);
...
console.log("=== Inequalities Generator Tests ===\n");
...
console.log("--- 5. Mixed pool config ---");
{
  ...
  mix.forEach(q => {
    assert(q.topic === "inequalities", `mixed: topic is inequalities`);
...
console.log("--- 8. Sign flip in hard questions ---");
{
  const hardPool = generatePool("hard", 20, 42);
  const signFlipQuestions = hardPool.filter(q =>
    q.metadata.skills.includes("flip_inequality_sign")
  );
  const pct = signFlipQuestions.length / hardPool.length;
  assert(signFlipQuestions.length >= 1, `at least 1 sign-flip question in hard pool (got ${signFlipQuestions.length})`);
  assert(pct >= 0.4, `at least 40% sign-flip in hard pool (got ${(pct * 100).toFixed(0)}%)`);
}

console.log("--- 9. Inequality signs in answers ---");
{
  for (const diff of ["easy", "medium", "hard"] as const) {
    const pool = generatePool(diff, 10, 321);
    for (const q of pool) {
      const hasSign = /[>≥<≤]/.test(q.answer);
      assert(hasSign, `${diff}/${q.archetype}: answer contains inequality sign: "${q.answer}"`);
    }
  }
}
