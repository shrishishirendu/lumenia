...
async function main() {
  ...
  for (const u of updates) {
    await client.query("UPDATE lesson_segments SET content = $1 WHERE id = $2", [u.content, u.id]);
    console.log(`Updated segment ${u.id}: ${u.content.length} chars, SVG: ${u.content.includes("<svg")}`);
  }
